...
const VERSION_BUILTIN: VersionInfo = { status: 'success', version: 'builtin' };

export class CapacitorUpdaterWeb
  extends WebPlugin
  implements CapacitorUpdaterPlugin {
  async download(options: { url: string }): Promise<VersionInfo> {
    console.warn('Cannot download version in web', options);
    return VERSION_BUILTIN;
  }
  async set(options: { version: string, versionName?: string }): Promise<VersionInfo> {
  ...
  async list(): Promise<{ versions: VersionInfo[] }> {
    ...
    return { versions: [VERSION_BUILTIN] };
  }
