...
@objc public class CapacitorUpdater: NSObject {
    ...
    public let tagLine: String = "✨  Capacitor-updater:"
    ...
    private func prepareFolder(source: URL) throws {
        if !FileManager.default.fileExists(atPath: source.path) {
            do {
                ...
                print("\(self.tagLine) Cannot createDirectory \(source.path)")
    ...
    private func deleteFolder(source: URL) throws {
        do {
            ...
            print("\(self.tagLine) File not removed. \(source.path)")
    ...
    private func unflatFolder(source: URL, dest: URL) throws -> Bool {
        ...
        do {
            ...
            print("\(self.tagLine) File not moved. source: \(source.path) dest: \(dest.path)")
    ...
    private func getChecksum(filePath: URL) -> String {
        do {
            ...
            print("\(self.tagLine) Cannot get checksum: \(filePath.path)", error)
            return ""
        }
    }
    ...
    private func decryptFile(filePath: URL, sessionKey: String, version: String) throws {
        if self.privateKey.isEmpty || sessionKey.isEmpty  || sessionKey.components(separatedBy: ":").count != 2 {
            print("\(self.tagLine) Cannot found privateKey or sessionKey")
            return
        }
        do {
            ...
            print("\(self.tagLine) Cannot decode: \(filePath.path)", error)
    ...
    public func getLatest(url: URL) -> AppVersion {
        ...
        print("\(self.tagLine) Auto-update parameters: \(parameters)")
        ...
        request.validate().responseDecodable(of: AppVersionDec.self) { response in
            ...
            case let .failure(error):
                print("\(self.tagLine) Error getting Latest", response.value ?? "", error )
    ...
    private func setCurrentBundle(bundle: String) {
        ...
        print("\(self.tagLine) Current bundle set to: \((bundle ).isEmpty ? BundleInfo.idBuiltin : bundle)")
    }
            ...
            if let fileURL = response.fileURL {
                ...
                case .success:
                    ...
                    do {
                        ...
                        print("\(self.tagLine) download unzip error", error)
                        mainError = error as NSError
                    }
                case let .failure(error):
                    print("\(self.tagLine) download error", response.value ?? "", error)
                    mainError = error as NSError
                }
            }
            semaphore.signal()
    ...
    public func list() -> [BundleInfo] {
        ...
        do {
            ...
            var res: [BundleInfo] = []
            print("\(self.tagLine) list File : \(dest.path)")
            if dest.exist {
            ...
            print("\(self.tagLine) No version available \(dest.path)")
            return []
        }
    }
    ...
    public func delete(id: String, removeInfo: Bool) -> Bool {
        ...
        if deleted.isBuiltin() || self.getCurrentBundleId() == id {
            print("\(self.tagLine) Cannot delete \(id)")
            return false
        }
        ...
        do {
            ...
            print("\(self.tagLine) Hot Folder \(destHot.path), not removed.")
        }
        do {
            try FileManager.default.removeItem(atPath: destPersist.path)
        } catch {
            print("\(self.tagLine) Folder \(destPersist.path), not removed.")
            return false
        }
        if removeInfo {
        ...
        print("\(self.tagLine) bundle delete \(deleted.getVersionName())")
    ...
    public func reset(isInternal: Bool) {
        print("\(self.tagLine) reset: \(isInternal)")
    ...
    public func setSuccess(bundle: BundleInfo, autoDeletePrevious: Bool) {
        ...
        print("\(self.tagLine) Fallback bundle is: \(fallback.toString())")
        print("\(self.tagLine) Version successfully loaded: \(bundle.toString())")
        if autoDeletePrevious && !fallback.isBuiltin() {
            let res = self.delete(id: fallback.getId())
            if res {
                print("\(self.tagLine) Deleted previous bundle: \(fallback.toString())")
            } else {
                print("\(self.tagLine) Failed to delete previous bundle: \(fallback.toString())")
            }
        }
    ...
    func setChannel(channel: String) -> SetChannel {
        ...
        if (self.channelUrl ).isEmpty {
            print("\(self.tagLine) Channel URL is not set")
        ...
        request.validate().responseDecodable(of: SetChannelDec.self) { response in
            ...
            case let .failure(error):
                print("\(self.tagLine) Error set Channel", response.value ?? "", error)
    ...
    func getChannel() -> GetChannel {
        ...
        request.validate().responseDecodable(of: GetChannelDec.self) { response in
            ...
            case let .failure(error):
                print("\(self.tagLine) Error get Channel", response.value ?? "", error)
    ...
    func sendStats(action: String, versionName: String) {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            request.responseData { response in
                switch response.result {
                case .success:
                    print("\(self.tagLine) Stats send for \(action), version \(versionName)")
                case let .failure(error):
                    print("\(self.tagLine) Error sending stats: ", response.value ?? "", error)
                }
            }
        }
    }
    ...
    public func getBundleInfo(id: String?) -> BundleInfo {
        ...
        // print("\(self.tagLine) Getting info for bundle [\(trueId)]")
        ...
        if BundleInfo.idBuiltin == trueId {
            ...
            do {
                ...
                print("\(self.tagLine) Failed to parse info for bundle [\(trueId)]", error.localizedDescription)
                result = BundleInfo(id: trueId, version: "", status: BundleStatus.PENDING, checksum: "")
            }
        }
        // print("\(self.tagLine) Returning info bundle [\(result.toString())]")
        return result
    }
    ...
    public func saveBundleInfo(id: String, bundle: BundleInfo?) {
        ...
        if bundle == nil {
            print("\(self.tagLine) Removing info for bundle [\(id)]")
            UserDefaults.standard.removeObject(forKey: "\(id)\(self.infoSuffix)")
        } else {
            let update = bundle!.setId(id: id)
            print("\(self.tagLine) Storing info for bundle [\(id)]", update.toString())
            do {
                try UserDefaults.standard.setObj(update, forKey: "\(id)\(self.infoSuffix)")
            } catch {
                print("\(self.tagLine) Failed to save info for bundle [\(id)]", error.localizedDescription)
            }
        }
        UserDefaults.standard.synchronize()
    }
    ...
    public func setVersionName(id: String, version: String) {
        print("\(self.tagLine) Setting version for folder [\(id)] to \(version)")
        let info = self.getBundleInfo(id: id)
        self.saveBundleInfo(id: id, bundle: info.setVersionName(version: version))
    }

    private func setBundleStatus(id: String, status: BundleStatus) {
        print("\(self.tagLine) Setting status for bundle [\(id)] to \(status)")
        let info = self.getBundleInfo(id: id)
