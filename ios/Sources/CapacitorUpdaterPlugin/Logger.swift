...
public class Logger {
    ...
    public init(
        ...
        if let config = config {
            ...
            if let configLabels = loggerPluginConfig("labels", from: config) as? [String: String] {
                labels = configLabels
            }

            if let configSyslog = loggerPluginConfig("useSyslog", from: config) as? Bool {
    ...
    private func loggerPluginConfig(_ configKey: String, from config: InstanceConfiguration) -> Any? {
