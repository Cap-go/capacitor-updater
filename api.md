...
| **`updateKind`** | `UpdateKind` | Configure when live updates should be applied to the app. Supported values: - `'background'`: Apply updates automatically when the app is moved to background (default). - `'foreground'`: Apply updates while the app is in the foreground. - `'appBoot'`: Apply downloaded and pending update bundles during initial app boot before UI rendering. Only available for Android and iOS. | `'background'` |  |
