...
| **`autoSplashscreenLoader`** | <code>boolean</code>                            | Display a native loading indicator on top of the splashscreen while automatic direct updates are running. Requires `autoSplashscreen` to be enabled and the @capacitor/splash-screen plugin to be installed. Only available for Android and iOS.                                                                     | <code>false</code>                                                            | 7.19.0  |
| **`autoSplashscreenTimeout`** | <code>number</code>                            | Automatically hide the splashscreen after the specified number of milliseconds when automatic direct updates are running. Updates continue in the background when the timeout elapses and the direct update is skipped, so the new bundle activates on the next background/launch. Set to `0` to disable. Requires `autoSplashscreen` to be enabled. Only available for Android and iOS. | <code>10000 // (10 seconds)</code>                                            | 7.19.0  |
...
```json
{
  "plugins": {
    "CapacitorUpdater": {
      ...
      "autoSplashscreenTimeout": 10000 // (10 seconds),
